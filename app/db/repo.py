...
from app.db.models import AnalyticsEvent, Device, Payment, PromoCode, PromoRedemption, Referral, Server, User
...
async def log_event(session: AsyncSession, user_id: int | None, event_type: str, meta: dict | None = None) -> AnalyticsEvent:
    event = AnalyticsEvent(user_id=user_id, event_type=event_type, meta_json=json.dumps(meta or {}, ensure_ascii=False))
    session.add(event)
    await session.commit()
...
async def list_events_counts(session: AsyncSession, start_at: dt.datetime, end_at: dt.datetime) -> dict[str, int]:
    result = await session.execute(
        select(AnalyticsEvent.event_type, func.count(func.distinct(AnalyticsEvent.user_id)))
        .where(AnalyticsEvent.created_at.between(start_at, end_at))
...
async def list_last_event_counts(session: AsyncSession, start_at: dt.datetime, end_at: dt.datetime) -> dict[str, int]:
    subquery = (
        select(
            AnalyticsEvent.user_id,
            func.max(AnalyticsEvent.created_at).label("max_dt"),
        )
        .where(AnalyticsEvent.created_at.between(start_at, end_at))
        .where(AnalyticsEvent.user_id.is_not(None))
    ...
    result = await session.execute(
        ...
        .join(subquery, (AnalyticsEvent.user_id == subquery.c.user_id) & (AnalyticsEvent.created_at == subquery.c.max_dt))
        .group_by(AnalyticsEvent.event_type)
    )
    return {row[0]: row[1] for row in result.all()}


async def list_payments_in_period(session: AsyncSession, start_at: dt.datetime, end_at: dt.datetime) -> list[Payment]:
    result = await session.execute(
        select(Payment)
        .where(Payment.status == "succeeded")
        .where(Payment.created_at.between(start_at, end_at))
    )
    return list(result.scalars().all())


async def list_users_created_in_period(session: AsyncSession, start_at: dt.datetime, end_at: dt.datetime) -> int:
    result = await session.execute(
        select(func.count(User.id)).where(User.created_at.between(start_at, end_at))
    )
    return int(result.scalar_one())


async def list_promo_redemptions_in_period(session: AsyncSession, start_at: dt.datetime, end_at: dt.datetime) -> list[PromoRedemption]:
    result = await session.execute(
        select(PromoRedemption).where(PromoRedemption.redeemed_at.between(start_at, end_at))
    )
    return list(result.scalars().all())
